        ...
        @foreach($products as $product)
            <div id="productvalues" wire:key="{{ $product['UID'] }}">
                ...
                <div class="numberofitems">
                    <p style="font-family: 'Poppins', sans-serif; "><span>{{$product['prd_name']}}</span> - <span id="mumberofpills"></span>{{$product['pills']}}<span> pills,</span><br>
                    <span id="pillsmg">{{$product['mg']}}</span><span> mg</span> </p>
                    <p><span id="itemsvalues">{{$product['qty']}}</span> <span>X</span> <span>$ <span id="productprice">{{$product['price']}}</span>
                    <span>=</span> <span>$</span> <span id="totalprice">{{ $product['qty'] * $product['price'] }}</span> </span></p>
                </div>
